...
const PokemonEdit: React.FC<PokemonEditProps> = ({ match }) => {
  ...
  useEffect(() => {
    api.get(`/pokemon/${id}`).then((response) => {
      ...
      formRef.current?.setFieldValue('img_name', response.data?.image_url);
    });
  }, [id]);
  ...
  const handleSubmit = useCallback(
    async (data: PokemonFormData) => {
      try {
        ...
        await api.put(`/pokemon/${id}`, dataPokemon);
